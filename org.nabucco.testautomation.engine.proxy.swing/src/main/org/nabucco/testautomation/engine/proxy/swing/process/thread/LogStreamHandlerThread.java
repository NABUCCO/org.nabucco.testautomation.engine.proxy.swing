...
/**
 * Monitors the external process System.out stream for occurrences and logs them.
 * 
 * @author Nicolas Moser, PRODYNA AG
 */
public class LogStreamHandlerThread extends Thread {

    private static final NBCTestLogger logger = NBCTestLoggingFactory.getInstance().getLogger(
            LogStreamHandlerThread.class);
     ...
     */
    public LogStreamHandlerThread(InputStream in) {
    ...
    public void run() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                logger.debug("External log: ", line);
            }
        } catch (Exception e) {
            logger.fatal(e, "Exception reading log stream.");
        }
    }
}
